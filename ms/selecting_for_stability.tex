...
\title{\textbf{Selecting for Stability} \\[6pt]
  \large Choose the Model Closest to the Ensemble}
...
Using a factorial simulation over 22 data-generating process configurations varying class separation, label noise, number of classes, sample size, and feature redundancy, we find that ensemble proximity selection reduces out-of-sample prediction variance by a mean of 7.4\% (median 7.6\%, 95\% CI $[-0.7\%, +14.9\%]$), with reductions of 15--20\% on hard problems, at essentially zero accuracy cost (mean gap: $+0.004$ percentage points).
The benefit is conditional: it scales with baseline prediction disagreement among models and is negligible when models already agree.
Validation-set proximity to the ensemble is a reliable predictor of test-set proximity (mean Spearman $\rho = 0.50$, significant in 41\% of configurations), confirming the criterion transfers out of sample.
Ensemble proximity outperforms alternative selection criteria including random selection, calibration-based selection, and entropy-based selection.
\end{abstract}
...
\citet{rudin2024amazing} recently emphasized the opportunity this creates for model selection beyond accuracy.
Our work navigates the Rashomon set using prediction stability as the secondary criterion.

\paragraph{Predictive multiplicity.}
\citet{marx2020predictive} formalized the phenomenon where equally accurate models produce conflicting predictions on individual examples, calling it ``predictive multiplicity.''
\citet{watson2023predictive} extended this framework to probabilistic classifiers.
Both works treat multiplicity as a diagnostic or fairness concern; we treat it as a selection opportunity.
The ensemble-proximate model minimizes a form of multiplicity: it is the model least likely to disagree with the consensus.

\paragraph{Stable retraining.}
\citet{milani2024stable} pursue the same goal---stable predictions under retraining---but via a different mechanism: they modify the training objective to penalize prediction variance explicitly.
Our approach is post-hoc: we select from an existing pool without modifying training.
The methods are complementary; their regularization approach could reduce variance within the pool from which we select.
...
To map out when ensemble proximity helps, we run a factorial simulation over 22 data-generating process (DGP) configurations, varying one factor at a time from a baseline ($n{=}1{,}000$, $p{=}30$, 10 informative features, 10 redundant, 4 classes, 5\% label noise, class separation 1.0):
...
\begin{figure}[t]
  ...
  \caption{Factor sweeps.
    Blue bars: variance reduction from ensemble proximity selection (error bars: 95\% bootstrap CIs).
    Red line: baseline prediction disagreement rate.
    Panel (f): across all 22 DGP configurations, disagreement predicts variance reduction ($\rho = +0.29$).}
  \label{fig:factorial}
\end{figure}

Figure~\ref{fig:factorial} shows the results.
Class separation is the dominant factor: at separation 0.3, variance reduction is $+20.1\%$; at 1.5, it remains positive at $+5.1\%$.
Sample size matters non-monotonically: very small samples ($n{=}300$) produce the highest variance and thus the largest reduction ($+19.9\%$), but medium samples ($n{=}2{,}000$) also show substantial benefit ($+10.6\%$), likely because the MLP still finds distinct local optima.
Number of classes scales the benefit: 3 classes yield $+15.3\%$, 2 classes near zero.

The key integrative finding is in panel (f): across all 22 configurations, baseline prediction disagreement among models correlates with the variance reduction from ensemble proximity selection ($\rho = +0.29$).
This gives practitioners a diagnostic: compute the disagreement rate among your model pool; higher disagreement suggests greater potential for variance reduction.

\begin{table}[ht]
  \centering
  \caption{Summary across all 22 factorial DGP configurations.}
  \label{tab:summary}
  \input{../tabs/tab_summary}
\end{table}

Table~\ref{tab:summary} provides the aggregate statistics with bootstrap confidence intervals.
The mean variance reduction is $+7.4\%$ (median $+7.6\%$, 95\% CI $[-0.7\%, +14.9\%]$), positive in 18 of 22 configurations (82\%).
The mean effect size (Cohen's $d = 0.12$) is small but consistent.
The mean accuracy cost is $+0.004$ percentage points---effectively zero, confirming that ensemble proximity operates orthogonally to accuracy.
Validation-set proximity transfers to the test set with mean Spearman $\rho = 0.50$; after FDR correction, 5 of 22 configurations (23\%) show significant transfer.
...
\subsection{Comparison to Alternative Selection Criteria}
\label{sec:baselines}

A natural question is whether ensemble proximity outperforms other plausible selection criteria.
We compare against five alternatives: (1) \emph{Random}: uniformly sample from the Rashomon set; (2) \emph{Minimum ECE}: select the best-calibrated model; (3) \emph{Maximum Entropy}: select the model with highest predictive uncertainty; (4) \emph{Median Disagreement}: select the model at median pairwise disagreement; (5) \emph{Weight Averaging}: average the weights of all Rashomon models.

\begin{table}[ht]
  \centering
  \caption{Comparison of selection criteria across all 22 DGP configurations.
    Variance reduction is relative to best validation accuracy baseline.
    Ensemble proximity achieves the best variance reduction among singleton-selection methods.}
  \label{tab:baselines}
  \input{../tabs/tab_baseline_comparison}
\end{table}

Table~\ref{tab:baselines} shows the results.
Ensemble proximity achieves the largest variance reduction (+7.7\%) among selection methods that deploy a single trained model.
Random selection performs worse than best-val (-1.9\%), confirming that random exploration of the Rashomon set increases variance.
Calibration-based selection (minimum ECE) increases variance, while uncertainty-based selection (maximum entropy) provides modest benefits (+4.0\%) but less than ensemble proximity.
Weight averaging achieves the lowest variance but at catastrophic accuracy cost (mean accuracy 0.34 vs.\ 0.65), confirming that naive weight averaging does not work for models trained from different initializations---a known limitation when models occupy different loss basins \citep{fort2020deep}.


\subsection{Robustness Analysis}
\label{sec:robustness}

We test whether ensemble proximity selection is sensitive to hyperparameter choices.

\begin{table}[ht]
  \centering
  \caption{Robustness checks.
    Results are stable across Rashomon thresholds, divergence metrics, optimizers, and learning rates.}
  \label{tab:robustness}
  \input{../tabs/tab_robustness}
\end{table}

Table~\ref{tab:robustness} shows that variance reduction remains positive and similar across:
(1) Rashomon threshold $\varepsilon \in \{0.02, 0.05\}$;
(2) divergence metrics (L2, KL divergence);
(3) optimizers (SGD, Adam);
(4) learning rates spanning an order of magnitude.
The effect is robust to reasonable hyperparameter variation.


\subsection{Architecture Generalization}
\label{sec:architectures}

We test whether ensemble proximity helps across different MLP architectures.

\begin{table}[ht]
  \centering
  \caption{Architecture comparison.
    Ensemble proximity helps both shallow and deep MLPs.}
  \label{tab:architectures}
  \input{../tabs/tab_architectures}
\end{table}

Table~\ref{tab:architectures} shows that ensemble proximity selection reduces variance for both standard and deep MLPs (+8.2\% and +5.6\% respectively).
The benefit generalizes across network depths.
...
Across 22 DGP configurations, the mean accuracy gap is $+0.004$ percentage points (Table~\ref{tab:summary}).
...
The benefit averages 7\% variance reduction across diverse data-generating processes, reaches 15--20\% on hard problems, and costs essentially nothing in accuracy.
The criterion is cheap (no additional training), interpretable (``select the most representative model''), and outperforms alternative selection criteria including random sampling, calibration-based selection, and entropy-based selection.
